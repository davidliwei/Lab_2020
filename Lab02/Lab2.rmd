...
author: "Wei Li, modified from slides from Qing Zhang"
...
- What is Pegasus? Lots of computers stringed together
...
- Can't just run stuff through the terminal on Pegasus, you will need to submit job using srun or sbatch (preferred)
...
- Pegasus might also be slow if everyone in the class tries at
the last minute.
...
- Details: [https://colonialone.gwu.edu/pegasus/](https://colonialone.gwu.edu/pegasus/)
...
Pegasus uses **slurm** workload manager.

- Submit a batch serial job: 	`sbatch runscript.sh`
- Run a script or application interactively: `srun --pty -p tiny -t 10 --mem 1000 /bin/bash [script or app]`
- Start interactive session: `srun --pty -p tiny -t 10 --mem 1000 /bin/bash`
...
---
- How to run (basic):
    - Replace `ref.fasta` and `read1.fastq` with the path on Pegasus.
